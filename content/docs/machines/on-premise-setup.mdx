...
ComfyDeploy supports multiple authentication methods for self-hosted machines:

#### JWT-Based Authentication
ComfyDeploy uses JWT tokens for secure authentication. The custom-node repository implements JWT parsing:

```typescript
// JWT token verification with secret
const decoded = jwt.verify(token, process.env.JWT_SECRET!);
```

#### Bearer Token Authentication
API requests use Bearer token authentication in the Authorization header:

```python
# Bearer token authentication in custom routes
...
#### Basic Authentication for Classic Machines
Classic machines use basic authentication with base64-encoded auth tokens:
...
Protect your ComfyUI endpoint using ngrok's traffic policy expressions:
...
Update your machine configuration in ComfyDeploy:
