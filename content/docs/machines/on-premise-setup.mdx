...
2. Click **Create Machine** → **Self Hosted Machine**
...
For secure deployments, authentication is handled at the API level, not through ComfyUI CLI parameters:

```bash
# Authentication is managed through ComfyDeploy's API integration
# No additional CLI parameters needed for ComfyUI startup
```
...
ComfyDeploy sends authenticated requests to your self-hosted runners using different methods based on machine type:

#### Authentication Flow: ComfyDeploy → Runner

**Classic Machines (with Ngrok)**
- ComfyDeploy sends requests with Basic authentication to your ngrok-protected endpoint
- Ngrok validates the authentication using traffic policy expressions before forwarding to ComfyUI
- Your runner receives the authenticated request from ComfyDeploy via ngrok

```bash
# ComfyDeploy sends requests like this to your classic machine:
...
**RunPod Serverless Machines**
- ComfyDeploy sends requests with RunPod-specific authentication
- RunPod handles the authentication and forwards to your serverless function

#### Custom Node Authentication Verification
Your ComfyUI instance with ComfyDeploy custom node can verify incoming requests:

```python
# Custom routes handle incoming authenticated requests from ComfyDeploy
...
Ngrok allows you to expose your local ComfyUI instance to the internet securely, making it accessible to ComfyDeploy without requiring port forwarding or public IP configuration.
...
export NGROK_DOMAIN="your-custom-domain.ngrok.app"  # Optional: for custom domains
```

### 3. Start ComfyUI with Ngrok Tunnel
...
ngrok http 8188 --domain=${NGROK_DOMAIN}
```

### 4. Ngrok Authentication Protection

Protect your ComfyUI endpoint using ngrok's traffic policy expressions to validate requests from ComfyDeploy:
...
This configuration ensures that only ComfyDeploy with the correct authentication token can reach your ComfyUI instance through the ngrok tunnel.

### 5. Programmatic Ngrok Setup

For automated setups, use the ngrok Node.js integration like ComfyDeploy's test environment:
...
### 6. Update ComfyDeploy Machine Configuration

Configure your machine in ComfyDeploy so it can send authenticated requests to your ngrok endpoint:
...
ComfyDeploy will use this `auth_token` to authenticate requests sent to your `endpoint`. The token should match the one configured in your ngrok traffic policy expressions.

### 7. Load Balancing with Multiple Tunnels

For high availability, you can set up multiple ngrok tunnels with load balancing:
...
tunnels:
  comfyui-1:
    addr: 8188
    proto: http
    domain: comfyui-1.ngrok.app
  comfyui-2:
    addr: 8189
    proto: http
    domain: comfyui-2.ngrok.app
```
...
6. **Secure your endpoints** using ngrok authentication expressions to protect your instances
